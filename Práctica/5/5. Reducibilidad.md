...
## $L_1 = \{0^n 1 \mid n \ge 0\}$

## $L_2 = \{1^n 0 \mid n \ge 0\}$
...
## $\text{HP} = \{(\langle M\rangle,w) \mid \text{M se detiene con input w}\}$

## $L_u = \{(\langle M\rangle,w) \mid \text{M acepta w}\}$
...
## $\text{HP}_\lambda = \{\langle M\rangle \mid \text{M se detiene con input } \lambda\}$

## Demuestre que existe una reducción $\text{HP}$ $\alpha$ $\text{HP}_\lambda$

## 5. Demuestre que $L_V ∉ RE$:

## $L_V=\{(\langle M\rangle) \mid L(M) = ∅\}$.

## Considere que si $\langle M\rangle$ es un código inválido de máquina de Turing también pertence a $L_V$ (ya que no reconoce ningún string). Así $L_V$ es el complemento del lenguaje $L_{NV} = \{(\langle M\rangle) \mid L(M) \neq \emptyset\}$
